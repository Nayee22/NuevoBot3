...
await this.sendMessage(m.chat, { text: `ESTE GRUPO *${m.messageStubParameters[0] == 'on' ? 'FUE CERRADO CON ÉXITO ✅🔒' : 'FUE ABIERTO CON ÉXITO ✅🔓'}*\n ${m.messageStubParameters[0] == 'on' ? 'AHORA SIGNIFICA QUE ES EL MOMENTO ADMIN , SOLO ADMINS PUEDEN ESCRIBIR' : 'EL TIEMPO DEL ADMIN TERMINO , AHORA TODOS PUEDEN ESCRIBIR'} EN ESTE GRUP`, mentions: [m.sender] }, { quoted: fkontak })
} else if (m.messageStubType == 29) {
await this.sendMessage(m.chat, { text: `@${m.messageStubParameters[0].split`@`[0]} YA ES ADMIN DE ESTE GRUPO ✅\n\nLA ACCIÓN FUE REALIZADA CON ÉXITO POR: ${usuario}`, mentions: [`${m.sender}`,`${m.messageStubParameters[0]}`], mentions: (await conn.groupMetadata(m.chat)).participants.map(v => v.id) }, { quoted: fkontak })
} else if (m.messageStubType == 30) {
await this.sendMessage(m.chat, { text: `@${m.messageStubParameters[0].split`@`[0]} ABANDONO EL GRUPO DE ADMINS JAJAJA 😂\n\nLA ACCIÓN FUE REALIZADA CON ÉXITO POR: ${usuario}`, mentions: [`${m.sender}`,`${m.messageStubParameters[0]}`], mentions: (await conn.groupMetadata(m.chat)).participants.map(v => v.id) }, { quoted: fkontak })
